document.addEventListener('DOMContentLoaded', function () {
    ...
    function initCustomCursor() {
        const cursor = document.querySelector('.nks-cursor');
        const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

        if (!cursor || prefersReduced || window.innerWidth < 768) {
            return;
        }

        const hoverTargets = document.querySelectorAll('a, button, [role="button"], [data-cursor-link]');
        ...
        document.body.classList.add('nks-cursor-enabled');

        const handleMouseMove = (event) => {
            targetX = event.clientX;
            targetY = event.clientY;

            if (!cursor.classList.contains('nks-cursor--visible')) {
                cursor.classList.add('nks-cursor--visible');
            }
        };

        const handleMouseLeave = () => {
            cursor.classList.remove('nks-cursor--visible');
        };
        ...
        hoverTargets.forEach((target) => {
            target.addEventListener('mouseenter', () => {
                cursor.classList.add('nks-cursor--link');
            });
            target.addEventListener('mouseleave', () => {
                cursor.classList.remove('nks-cursor--link');
            });
        });
    }
